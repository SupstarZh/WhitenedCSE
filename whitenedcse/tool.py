...
class WhitenedCSE(object):
    """
    A class for embedding sentences, calculating similarities, and retriving sentences by WhitenedCSE.
    """
...
if __name__=="__main__":
    ...
    model_name = "princeton-nlp/sup-wcse-bert-base-uncased"
    wcse = WhitenedCSE(model_name)

    print("\n=========Calculate cosine similarities between queries and sentences============\n")
    similarities = wcse.similarity(example_queries, example_sentences)
    print(similarities)

    print("\n=========Naive brute force search============\n")
    wcse.build_index(example_sentences, use_faiss=False)
    results = wcse.search(example_queries)
    ...
    wcse.build_index(example_sentences, use_faiss=True)
    results = wcse.search(example_queries)
    for i, result in enumerate(results):
        print("Retrieval results for query: {}".format(example_queries[i]))
        for sentence, score in result:
